...
async def generate_nftables(db) -> str:
    ...
    if nat_inbound or nat_outbound:
        ...
        # Explicit outbound NAT
        for n in nat_outbound:
            lines.append(f'        # NAT (SNAT/MASQUERADE) {n.get("description", "")}')
            iface_dev = next((i['device'] for i in interfaces if i['name'] == n['interface']), n['interface'].lower())
            lines.append(f'        oifname "{iface_dev}" masquerade')
        # Fallback masquerade for WAN if no outbound rule defined
        if not nat_outbound:
...
async def generate_iptables(db) -> str:
    ...
    for n in nat_outbound:
        iface_dev = next((i['device'] for i in interfaces if i['name'] == n['interface']), n['interface'].lower())
        lines.append(f'# NAT (SNAT/MASQUERADE) {n.get("description", "")}')
        lines.append(f'iptables -t nat -A POSTROUTING -o {iface_dev} -j MASQUERADE')
    if not nat_outbound:
        for iface in interfaces:
            if iface.get('role') == 'wan':
